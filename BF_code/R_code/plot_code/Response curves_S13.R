...
## Load data
data <- read.csv("Trend_all_0901.csv", header=TRUE)     ## load csv file
head(data)

#################################################################################################################################     NPP_S

model <- glm(NPP_M ~  H_mean + TMP_mean + PRE_mean + FIRE_mean + POP_mean + DEM + S_CEC + S_OC + S_SAND, data = data)
summary(model)

# 预测值及其置信区间
predictions <- predict(model, interval = "confidence", level = 0.95)
predicted_df <- as.data.frame(predictions)
predicted_data <- cbind(predicted_df, data[c("H_mean", "TMP_mean", "PRE_mean", "FIRE_mean", "POP_mean", "DEM", "S_CEC", "S_OC", "S_SAND")])
cor(predicted_data)
head(predicted_data)
...
plot_NPP_S <- plot_grid(H_mean, TMP_mean, PRE_mean, FIRE_mean, POP_mean, DEM, S_CEC, S_OC, S_SAND,
...
model <- glm(kNDVI_M ~  H_mean + TMP_mean + PRE_mean + FIRE_mean + POP_mean + DEM + S_CEC + S_OC + S_SAND, data = data)
summary(model)
...
FIRE_mean <- ggplot(data, aes(x = FIRE_mean, y = kNDVI_M)) +
  #geom_point()+
  geom_smooth(method = "glm",  color="black",  se= TRUE, fill = "#36ACA2", size = 1.5) +
  ylim(0.1, 0.8) + xlim(0, 1000000) +
  theme_linedraw(base_size = 40)+
  theme(text = element_text(size = 30), axis.text = element_blank(),
        axis.title.y = element_blank(), axis.title.x = element_blank(),
        panel.grid.minor = element_blank(),
...
POP_mean <- ggplot(data, aes(x = POP_mean, y = kNDVI_M)) +
  #geom_point()+
  geom_smooth(method = "glm",  color="black",  se= TRUE, fill = "#36ACA2", size = 1.5) +
  ylim(0.1, 0.8) + xlim(0, 1) +
  theme_linedraw(base_size = 40)+
  theme(text = element_text(size = 30), axis.text = element_blank(),
        axis.title.y = element_blank(), axis.title.x = element_blank(),
        panel.grid.minor = element_blank(),
...
plot_kNDVI <- plot_grid(H_mean, TMP_mean, PRE_mean, FIRE_mean, POP_mean, DEM, S_CEC, S_OC, S_SAND,
                        align="hv", label_size = 20, ncol = 9, nrow = 1, label_fontface = "plain")
plot_kNDVI
...
#################################################################################################################################     VOD_Ku_S
head(data)
model <- glm(VOD_Ku_M ~  H_mean + TMP_mean + PRE_mean + FIRE_mean + POP_mean + DEM + S_CEC + S_OC + S_SAND, data = data)
summary(model)
...
plot_VOD_Ku_S <- plot_grid(H_mean, TMP_mean, PRE_mean, FIRE_mean, POP_mean, DEM, S_CEC, S_OC, S_SAND,
                           align="hv", label_size = 20, ncol = 9, nrow = 1, label_fontface = "plain")
plot_VOD_Ku_S
...
#################################################################################################################################     Carbon_S
head(data)
model <- glm(Carbon_M ~  H_mean + TMP_mean + PRE_mean + FIRE_mean + POP_mean + DEM + S_CEC + S_OC + S_SAND, data = data)
summary(model)
...
plot_Carbon_S <- plot_grid(H_mean, TMP_mean, PRE_mean, FIRE_mean, POP_mean, DEM, S_CEC, S_OC, S_SAND,
                           align="hv", label_size = 20, ncol = 9, nrow = 1, label_fontface = "plain")
plot_Carbon_S
...
model <- glm(b1_Bioma_M ~ H_mean + TMP_mean + PRE_mean + FIRE_mean + POP_mean + DEM + S_CEC + S_OC + S_SAND, data = data)
summary(model)
...
plot_Biomass_S <- plot_grid(H_mean, TMP_mean, PRE_mean, FIRE_mean, POP_mean, DEM, S_CEC, S_OC, S_SAND,
                            align="hv", label_size = 20, ncol = 9, nrow = 1, label_fontface = "plain")
plot_Biomass_S
...
#################################################################################################################################     Stabi_S
head(data)
model <- glm(Stab_M ~ H_mean + TMP_mean + PRE_mean + FIRE_mean + POP_mean + DEM + S_CEC + S_OC + S_SAND, data = data)
summary(model)
...
plot_Stabi_S <- plot_grid(H_mean, TMP_mean, PRE_mean, FIRE_mean, POP_mean, DEM, S_CEC, S_OC, S_SAND,
                          align="hv", label_size = 20, ncol = 9, nrow = 1, label_fontface = "plain")
plot_Stabi_S
...
ggsave("plot_mean3.jpg",device = "jpg",width = 1200, height = 800, units = "mm", dpi = 600)
